...
void initTestData(){
  envData.novaPm25 = 10.0 + bulkCount;
   envData.novaPm10 = 20.0 + bulkCount ;
   envData.temperature = 25.0 + bulkCount;
   envData.humidity = 45.0 + bulkCount;
   envData.time = now();
   //printData(&envData);
...
void setup() {
  ...
  strcpy(hubConfig.macStr ,"34-A2-33-01-5D-3");
        ...
        if (isValidAirData(envData)) {
          ...
          if (WiFi.status() == WL_CONNECTED) {
            ...
            if (ok) {
              ...
              sendBulkData(&restProperty);
            } else {
              saveBulkData(&envData);
...
void readBulkData(){
  ...
  Serial.print("Reading saved data from EEPROM: ");
  Serial.println(bulkCount);
  index +=  sizeof(bulkCount);
  
  for(int i = 0; i < bulkCount; i++){
    index +=  i * sizeof(envData);
    Serial.print("Getting data from index: " );
    Serial.println(index);
    EEPROM.get(index,bulkData[i]);
  }
  
}
void saveBulkData(Environment * envRecord){
  Serial.println("SAVING BULK DATA");
  if (bulkCount < BULK_CAPACITY){
              
    //memcpy(&bulkData[bulkCount],&envRecord, sizeof(envData));
    bulkData[bulkCount] = envData;
    
    //saveBulkData(&bulkData[bulkCount]);
    int index = sizeof(hubConfig) ;
    
    EEPROM.put(index,bulkCount);
    index +=  sizeof(bulkCount) + bulkCount  * sizeof(envData);
    EEPROM.put(index,envData);
    EEPROM.commit();
    delay(50);
    bulkCount++;
    }else{
...
void printBulkData(){
  Serial.println("BULK DATA");
  Serial.print("BULK COUNT: ");
  Serial.println(bulkCount);
  for(int i = 0; i < bulkCount; i++){
    printData(&bulkData[i]);
    
  }
}
void sendBulkData(RestProperty * restProperty){
  for(int i = 0; i < bulkCount; i++){
    printData(&bulkData[i]);
    #if SNIFFER_TEST
    bool ok = saveData_staging(&bulkData[i], restProperty);
    #else
    bool ok = saveData(&bulkData[i], restProperty);
